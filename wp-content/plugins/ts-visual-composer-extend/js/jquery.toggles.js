...
Customized by Tekanewa Scripts
*/
(function( $ ){
    $.fn['TSToggleSwitch'] = function(options) {
        options = options || {};
        ...
        var opts = $.extend({
            ...
            'input': null,                  // the input to set (for use in forms)
        ...
        var doToggle = function(slide, width, height, state) {
            ...
            opts['checkbox'].prop('checked', active).attr('checked', active);
            
            opts['input'].val(active);
            opts['input'].trigger("change");
            //opts['input'].onchange();
